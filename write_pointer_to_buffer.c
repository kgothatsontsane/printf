...
/**
 * write_pointer_to_buffer - Outputs a memory address to a character buffer with formatting.
...
int write_pointer_to_buffer(char buf[], int index, int len,
				  int w, int flgs, char padding, char extrac, int padstart)
{
	int i;
	...
	if (w > len)
	{
		for (i = 3; i < w - len + 3; i++)
			buf[i] = padding;
		buf[i] = '\0';
		if (flgs & F_MINUS && padding == ' ') /* Asign extra char to left of buf */
		{
			buf[--index] = 'x';
			buf[--index] = '0';
			if (extrac)
				buf[--index] = extrac;
			return (write(1, &buf[index], len) + write(1, &buf[3], i - 3));
		}
		else if (!(flgs & F_MINUS) && padding == ' ') /* extra char to left of buf */
		{
			buf[--index] = 'x';
			buf[--index] = '0';
			if (extrac)
				buf[--index] = extrac;
			return (write(1, &buf[3], i - 3) + write(1, &buf[index], len));
		}
		else if (!(flgs & F_MINUS) && padding == '0') /* extra char to left of padding */
		{
			if (extrac)
				buf[--padstart] = extrac;
			buf[1] = '0';
			...
			return (write(1, &buf[padstart], i - padstart) +
					write(1, &buf[index], len - (1 - padstart) - 2));
		}
	}
	buf[--index] = 'x';
	buf[--index] = '0';
